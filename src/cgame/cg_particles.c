...
typedef enum {
	P_NONE, P_WEATHER, P_FLAT, P_SMOKE, P_ROTATE, // unused
	P_WEATHER_TURBULENT, P_ANIM, P_DLIGHT_ANIM, P_BLEED, P_FLAT_SCALEUP, P_FLAT_SCALEUP_FADE, P_WEATHER_FLURRY, P_SMOKE_IMPACT, P_BUBBLE, P_BUBBLE_TURBULENT, P_SPRITE
} particle_type_t;

#define MAX_SHADER_ANIMS        8
#define MAX_SHADER_ANIM_FRAMES  64
static char *shaderAnimNames[MAX_SHADER_ANIMS] = {
	"explode1", "blacksmokeanim", "twiltb2", "blacksmokeanimc", // "expblue", // "blacksmokeanimb", // uses 'explode1' sequence // pulled
	// "blood", NULL
};

static qhandle_t shaderAnims[MAX_SHADER_ANIMS][MAX_SHADER_ANIM_FRAMES];
static int shaderAnimCounts[MAX_SHADER_ANIMS] = {
	23, 23, // removing warning messages from startup
	45, 23, 25, 23, 5,
};
...
void CG_AddParticleToScene(cparticle_t *p, vec3_t org, float alpha) {
	...
	case P_DLIGHT_ANIM:
	{
		...
		if (p->type == P_DLIGHT_ANIM) {
			...
			trap_R_AddLightToScene(org, 320, // %	1.5 * (width > height ? width : height),  1.25 * (1.0 - ratio), 1.0f, 0.95f, 0.85f, 0, 0);
		}
...
=======================================================================================================================================
*/
void CG_Particle_BleedExtended(qhandle_t pshader, vec3_t origin, int lifetime, int vel, int acc, int maxroll, float alpha, float size) {
	cparticle_t *p;
...
=======================================================================================================================================
*/
void CG_Particle_Bleed(qhandle_t pshader, vec3_t origin) {
