...
=======================================================================================================================================
*/
static void CG_ClipMoveToEntities(const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int skipNumber, int mask, int capsule, trace_t *tr) {
	int i, x, zd, zu;
	trace_t trace;
	entityState_t *ent;
	clipHandle_t cmodel;
...
int CG_PredictionOk(playerState_t *ps1, playerState_t *ps2) {
	...
	if (ps2->legsTimer != ps1->legsTimer || ps2->legsAnim != ps1->legsAnim || ps2->torsoTimer != ps1->torsoTimer || ps2->torsoAnim != ps1->torsoAnim) {
		return 9;
	}
...
const char *predictionStrings[] = {
	"OK", "PM TYPE FLAGS TIME", "origin", "velocity", "eFlags", "weapon time", "groundEntityNum", "--", // 7
	"speed || delta_angles", "anim || timer", "--", // 10
	"eventSequence", "events || eventParms", "externalEvent", "clientNum", "weapon || weaponState", "viewangles", "viewheight", "damage event ...", "stats", "persistant", // 20
	"powerups", "ammo", "viewlocked", "onFireStart", "--", "--", "grenadeTimeLeft", // 27
};
...
void CG_PredictPlayerState(void) {
...
#ifdef ALLOW_GSYNC || cg_synchronousClients.integer
#endif // ALLOW_GSYNC
) {
	...
	// the server time is beyond our current cg.time, // because predicted player positions are going to
	...
	if (cg.time > cg.cameraShakeTime) {
		...
		// move
		cg.predictedPlayerState.origin[2] += sin(M_PI * 8 * 13 + cg.cameraShakePhase) * x * 6.0f * cg.cameraShakeScale;
		cg.predictedPlayerState.origin[1] += sin(M_PI * 17 * x + cg.cameraShakePhase) * x * 6.0f * cg.cameraShakeScale;
