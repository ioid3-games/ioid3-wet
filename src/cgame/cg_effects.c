...
void CG_GibPlayer(centity_t *cent, vec3_t playerOrigin, vec3_t gdir) {
	...
	{
		// legs tags
		"tag_legright", "tag_legleft", "tag_torso", "tag_torso",
	...
	{
		// tags in the legs
		"tag_footright", "tag_footleft", "tag_legright", "tag_legleft", "tag_torso",

		// tags in the torso
		"tag_chest", "tag_armright", "tag_armleft", "tag_head", NULL
	};
	...
	if (cg_blood.integer && cg_bloodTime.integer) {
		...
		for (i = 0, count = 0; i < GIB_BLOOD_DOTS * 2; i++) {
			...
			if (trace.fraction < 1.0) {
				...
				trap_R_ProjectDecal(cgs.media.bloodDotShaders[rand() % 5], 1, (vec3_t *)trace.endpos, projection, color,   cg_bloodTime.integer * 1000, (cg_bloodTime.integer * 1000) >> 4);
				#endif
...
void CG_RenderSmokeGrenadeSmoke(centity_t *cent, const weaponInfo_t *weapon) {
	...
	if (cent->currentState.effect1Time > 16) {
		...
		if (!cent->dl_atten || cent->currentState.pos.trType != TR_STATIONARY || (cent->currentState.groundEntityNum != ENTITYNUM_WORLD && !VectorCompare(cent->lastLerpOrigin, cent->lerpOrigin))) {
			trace_t tr;
