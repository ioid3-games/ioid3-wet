...
void Menu_UpdatePosition(menuDef_t *menu) {
	...
	for (i = 0; i < menu->itemCount; ++i) {
		...
		if ((fullscreenMenu && !fullscreenItem) || !Q_stricmp(menuName, "main") || !Q_stricmp(menuName, "ingame_main") || centered) {
			...
			else if (!Q_stricmp(itemName, "etl_logo") || !Q_stricmp(itemName, "credits_etlegacy")) {
...
void Menu_Paint(menuDef_t *menu, qboolean forcePaint) {
	...
	} else if (DC->getCVarValue("ui_showtooltips") && item != NULL && item->toolTipData != NULL && item->toolTipData->text != NULL && *item->toolTipData->text) {
		Item_Paint(item->toolTipData);
	}
	// handle timeout here
	if (menu->openTime == 0) {
		menu->openTime = DC->realTime;
	} else if ((menu->window.flags & WINDOW_VISIBLE) && menu->timeout > 0 && menu->onTimeout != NULL && menu->openTime + menu->timeout <= DC->realTime) {
		itemDef_t it;
		it.parent = menu;
