...
char *CheckUserinfo(int clientNum, char *userinfo) {
	...
	if (length > 4) {
		for (i = 0; userinfo[i + 3]; ++i) {
			if (userinfo[i] == '\\' && userinfo[i + 1] == 'i' && userinfo[i + 2] == 'p' && userinfo[i + 3] == '\\') {
				count++;
			}
		}
	}
	...
	if (length > 9) {
		for (i = 0; userinfo[i + 8]; ++i) {
			if (userinfo[i] == '\\' && userinfo[i + 1] == 'c' && userinfo[i + 2] == 'l' && userinfo[i + 3] == '_' && userinfo[i + 4] == 'g' && userinfo[i + 5] == 'u' && userinfo[i + 6] == 'i' && userinfo[i + 7] == 'd' && userinfo[i + 8] == '\\') {
				count++;
			}
		}
	}
	...
	if (length > 6) {
		for (i = 0; userinfo[i + 5]; ++i) {
			if (userinfo[i] == '\\' && userinfo[i + 1] == 'n' && userinfo[i + 2] == 'a' && userinfo[i + 3] == 'm' && userinfo[i + 4] == 'e' && userinfo[i + 5] == '\\') {
				count++;
			}
		}
	}
	...
	if (length > 15) {
		for (i = 0; userinfo[i + 14]; ++i) {
			if (userinfo[i] == '\\' && userinfo[i + 1] == 'c' && userinfo[i + 2] == 'l' && userinfo[i + 3] == '_' && userinfo[i + 4] == 'p' && userinfo[i + 5] == 'u' && userinfo[i + 6] == 'n' && userinfo[i + 7] == 'k' && userinfo[i + 8] == 'b' && userinfo[i + 9] == 'u' && userinfo[i + 10] == 's' && userinfo[i + 11] == 't' && userinfo[i + 12] == 'e' && userinfo[i + 13] == 'r' && userinfo[i + 14] == '\\') {
				count++;
			}
		}
	}
...
void ClientUserinfoChanged(int clientNum) {
	...
	while (1) {
		...
		*/
		case TOK_name:
			...
			break;
		case TOK_cl_guid:
			if (!(ent->r.svFlags & SVF_BOT) && strcmp(client->pers.cl_guid, "unknown") && strcmp(client->pers.cl_guid, cs_value)) {
	...
	if (ent->r.svFlags & SVF_BOT) {
		...
		if (cs_cg_uinfo[0]) {
			sscanf(cs_cg_uinfo, "%i %i %i", &client->pers.clientFlags, &client->pers.clientTimeNudge, &client->pers.clientMaxPackets);
		} else {
			// cg_uinfo was empty and useless.
			// Let us fill these values in with something useful, // if there not present to prevent auto reload
			// from failing. - forty
	...
	s = va("n\\%s\\t\\%i\\c\\%i\\lc\\%i\\r\\%i\\m\\%s\\s\\%s\\dn\\%i\\w\\%i\\lw\\%i\\sw\\%i\\mu\\%i\\ref\\%i\\u\\%u", client->pers.netname, client->sess.sessionTeam, client->sess.playerType, client->sess.latchPlayerType, client->sess.rank, medalStr, skillStr, client->disguiseClientNum, client->sess.playerWeapon, client->sess.latchPlayerWeapon, client->sess.latchPlayerWeapon2, client->sess.muted ? 1 : 0, client->sess.referee, client->sess.uci
);
		...
		if (ent->client->pers.connected != CON_DISCONNECTED) {
			...
			if (ipMatch == qtrue) {
				...
				if (count > max) {
					...
					return va("Only %d connection%s per IP %s allowed on this server!", max, max == 1 ? "" : "s", max == 1 ? "is" : "are");
				}
			}
		}
	}
...
char *ClientConnect(int clientNum, qboolean firstTime, qboolean isBot) {
	...
	if (gidb != NULL /*&& !isBot*/) {
		...
		if (!strcmp(cs_ip, "localhost")) {
			...
			if (((ip & 0xFF000000) == 0x0A000000) || ((ip & 0xFFF00000) == 0xAC100000) || ((ip & 0xFFFF0000) == 0xC0A80000) || (ip == 0x7F000001))                 // recognise also 127.0.0.1
			{
				client->sess.uci = 0;
			} else {
...
void ClientDisconnect(int clientNum) {
	...
	for (i = 0; i < level.numConnectedClients; i++) {
		...
		if (flag->client->sess.sessionTeam == TEAM_SPECTATOR && flag->client->sess.spectatorState == SPECTATOR_FOLLOW && flag->client->sess.spectatorClient == clientNum) {
			StopFollowing(flag);
		}
