...
static const char *gameNames[] = {
	...
	"Objective", "Stopwatch", "Campaign", "Last Man Standing", "Map Voting"       // GT_WOLF_MAPVOTE
	// GT_MAX_GAME_TYPE
};
...
static const vote_reference_t aVoteInfo[] = {
	{0x1ff, "gametype",   G_Gametype_v, "Set Gametype to", " <value> ^ 7\n  Changes the current gametype"                        }, {0x1ff, "kick", G_Kick_v,  "KICK",      " <player_id > ^ 7\n  Attempts to kick player from server"             }, {0x1ff, "mute", G_Mute_v,  "MUTE",      " <player_id > ^ 7\n  Removes the chat capabilities of a player"       }, {0x1ff, "unmute",  G_UnMute_v,   "UN - MUTE",   " <player_id > ^ 7\n  Restores the chat capabilities of a player"      }, {0x1ff, "map",  G_Map_v,   "Change map to",   " < mapname > ^ 7\n  Votes for a new map to be loaded"                  }, {0x1ff, "campaign",   G_Campaign_v, "Change campaign to", " < campaign > ^ 7\n  Votes for a new map to be loaded"                 }, {0x1ff, "maprestart", G_MapRestart_v, "Map Restart",  "^7\n  Restarts the current map in progress"                        }, {0x1ff, "matchreset", G_MatchReset_v, "Match Reset",  "^7\n  Resets the entire match"                                     }, {0x1ff, "mutespecs",  G_Mutespecs_v, "Mute Spectators", " < 0|1 > ^ 7\n  Mutes in - game spectator chat"                          }, {0x1ff, "nextmap", G_Nextmap_v,  "Load Next Map",   "^7\n  Loads the next map or campaign in the map queue"             }, {0x1ff, "referee", G_Referee_v,  "Referee",   " <player_id > ^ 7\n  Elects a player to have admin abilities"         }, {0x1ff, "shuffleteamsxp", G_ShuffleTeams_v, "Shuffle Teams by XP", "^7\n  Randomly place players on each team, based on XP"           }, {0x1ff, "shuffleteamsxp_norestart", G_ShuffleTeams_NoRestart_v, "Shuffle Teams by XP(No Restart)", "^7\n  Randomly place players on each team, based on XP"           }, {0x1ff, "startmatch", G_StartMatch_v, "Start Match",  "^7\n  Sets all players to \"ready\" status to start the match"    }, {0x1ff, "swapteams",  G_SwapTeams_v, "Swap Teams",   "^7\n  Switch the players on each team"                            }, {0x1ff, "friendlyfire", G_FriendlyFire_v, "Friendly Fire",   " < 0|1 > ^ 7\n  Toggles ability to hurt teammates"                     }, {0x1ff, "timelimit",  G_Timelimit_v, "Timelimit", " <value> ^ 7\n  Changes the current timelimit"                       }, {0x1ff, "unreferee",  G_Unreferee_v, "UNReferee", " <player_id > ^ 7\n  Elects a player to have admin abilities removed" }, {0x1ff, "warmupdamage", G_Warmupfire_v, "Warmup Damage",   " < 0|1|2 > ^ 7\n  Specifies if players can inflict damage during warmup"}, {0x1ff, "antilag", G_AntiLag_v,  "Anti - Lag",  " < 0|1 > ^ 7\n  Toggles Anit - Lag on the server"                        }, {0x1ff, "balancedteams", G_BalancedTeams_v, "Balanced Teams",  " < 0|1 > ^ 7\n  Toggles team balance forcing"                          }, {0x1ff, "surrender",  G_Surrender_v, "Surrender", "  ^ 7\n  Ends the match."                                           }, {0x1ff, "restartcampaign", G_RestartCampaign_v, "Restart Campaign",   "^7\n  Restarts the current Camapaign"                             }, {0x1ff, "nextcampaign", G_NextCampaign_v, "Next Campaign",   "^7\n  Ends the current campaign and starts the next one."         }, {0x1ff, "poll", G_Poll_v,  "[poll]",    " <text> ^ 7\n  Poll majority opinion."                               }, {0x1ff, "config",  G_Config_v,   "Game config",  " < configname > ^ 7\n  Loads up the server game config"                }, {0, 0,   NULL,   0}
};
...
void G_voteHelp(gentity_t *ent, qboolean fShowVote) {
	...
	for (i = 0; i < rows; i++) {
		if (i + rows * 3 + 1 <= num_cmds) {
			G_refPrintf(ent, "^5% - 25s% - 25s% - 25s% - 25s", aVoteInfo[vi[i]].pszVoteName, aVoteInfo[vi[i + rows]].pszVoteName, aVoteInfo[vi[i + rows * 2]].pszVoteName, aVoteInfo[vi[i + rows * 3]].pszVoteName);
		} else if (i + rows * 2 + 1 <= num_cmds) {
			G_refPrintf(ent, "^5% - 25s% - 25s% - 25s", aVoteInfo[vi[i]].pszVoteName, aVoteInfo[vi[i + rows]].pszVoteName, aVoteInfo[vi[i + rows * 2]].pszVoteName);
		} else if (i + rows + 1 <= num_cmds) {
			G_refPrintf(ent, "^5% - 25s% - 25s", aVoteInfo[vi[i]].pszVoteName, aVoteInfo[vi[i + rows]].pszVoteName);
		} else {
...
int G_Mutespecs_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd) {
	// Vote request(vote is being initiated)
	if (arg) {
		return (G_voteProcessOnOff(ent, arg, arg2, fRefereeCmd,   !!(match_mutespecs.integer),   vote_allow_mutespecs.integer,   dwVoteIndex));
		// Vote action(vote has passed)
	} else {
...
int G_FriendlyFire_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd) {
	// Vote request(vote is being initiated)
	if (arg) {
		return (G_voteProcessOnOff(ent, arg, arg2, fRefereeCmd,   !!(g_friendlyFire.integer),   vote_allow_friendlyfire.integer,   dwVoteIndex));
		// Vote action(vote has passed)
	} else {
...
int G_BalancedTeams_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd) {
	// Vote request(vote is being initiated)
	if (arg) {
		return (G_voteProcessOnOff(ent, arg, arg2, fRefereeCmd,   !!(g_balancedteams.integer),   vote_allow_balancedteams.integer,   dwVoteIndex));
		// Vote action(vote has passed)
	} else {
...
void G_IntermissionMapList(gentity_t *ent) {
	...
	for (i = 0; i < maxMaps; i++) {
		Q_strcat(mapList, MAX_STRING_CHARS, va("%s %d %d %d ", level.mapvoteinfo[level.sortedMaps[i]].bspName, level.sortedMaps[i], level.mapvoteinfo[level.sortedMaps[i]].lastPlayed, level.mapvoteinfo[level.sortedMaps[i]].totalVotes));
	}
...
int G_Surrender_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd) {
	...
	if (arg) {
		...
		Q_strncpyz(arg2, (ent->client->sess.sessionTeam == TEAM_AXIS) ?
		           "[AXIS]" : "[ALLIES]", VOTE_MAXSTRING);
	}
	// Vote action(vote has passed)
		...
		Info_SetValueForKey(cs, "w", (level.voteInfo.voteTeam == TEAM_AXIS) ? "1" : "0");
		trap_SetConfigstring(CS_MULTI_MAPWINNER, cs);
		G_LogExit(va("%s Surrender\n", (level.voteInfo.voteTeam == TEAM_AXIS) ?
		             "Axis" : "Allies"));
		AP(va("chat \"%s have surrendered!\"", (level.voteInfo.voteTeam == TEAM_AXIS) ?
		      "^1AXIS ^ 7" : "^4ALLIES ^ 7"));
	}
...
int G_RestartCampaign_v(gentity_t *ent, unsigned int dwVoteIndex, char *arg, char *arg2, qboolean fRefereeCmd) {
	...
	// Vote action(vote has passed)
		...
		trap_Cvar_VariableStringBuffer("nextcampaign", s, sizeof(s));
		trap_SendConsoleCommand(EXEC_APPEND, va("campaign %s%s\n",    g_campaigns[level.currentCampaign].shortname,   ((*s) ? va("; set nextcampaign \"%s\"", s) : "")));
	}
