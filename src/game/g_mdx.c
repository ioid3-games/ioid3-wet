...
static void mdx_calculate_bone(
    vec3_t dest, const struct bone *bone, const struct frame_bone	*frameBone
) {
	vec3_t tmp;
	vec3_t axis[3];
	...
	if (i == 0) {
		...
		if (recursive) {
			mdx_calculate_bone_lerp(
			    refent, frameModel, oldFrameModel, torsoFrameModel, oldTorsoFrameModel, bone->parent_index, qtrue
		);
		}
	}
...
#ifdef LEGACY_DEBUG
	if (frameModel->bone_count != torsoFrameModel->bone_count || frameModel->bone_count != oldFrameModel->bone_count || frameModel->bone_count != oldTorsoFrameModel->bone_count) {
		G_Error(GAME_VERSION " MDX: Frame count mismatch\n");
	}
#endif
	for (i = 0; i < frameModel->bone_count; i++) {
		mdx_calculate_bone_lerp(
		    refent, frameModel, oldFrameModel, torsoFrameModel, oldTorsoFrameModel, i, qfalse
	);
	}
}
#endif // BONE_HITTESTS
...
#endif
	mdx_calculate_bone_lerp(
	    refent, frameModel, oldFrameModel, torsoFrameModel, oldTorsoFrameModel, i, qtrue
);
}
...
static void mdx_bone_orientation(/*const*/ grefEntity_t *refent, int idx, vec3_t origin, vec3_t axis[3]) {
	...
	// FIXME: This probably isn't entirely correct; my test models fail, // in any case. It seems to produce the proper results with a real
	// player model, though.
...
static qboolean mdx_hit_warp(
    const vec3_t start, const vec3_t end, const vec3_t origin, 
    /*const*/ vec3_t axis[3], const vec3_t scale, 
