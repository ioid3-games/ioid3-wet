...
void SpectatorThink(gentity_t *ent, usercmd_t *ucmd) {
	...
	if (crosshairEnt->inuse && crosshairEnt->client && (ent->client->sess.sessionTeam == crosshairEnt->client->sess.sessionTeam ||
	...
	if ((client->buttons & BUTTON_ATTACK) && !(client->oldbuttons & BUTTON_ATTACK) && client->sess.spectatorState != SPECTATOR_FOLLOW && client->sess.sessionTeam == TEAM_SPECTATOR)     // don't do it if on a team
	{
		if (G_SpectatorAttackFollow(ent)) {
			return;
		} else {
			// not clicked on a player?.. then follow next, // to prevent constant traces done by server.
	...
	} else if ((client->buttons & BUTTON_ATTACK) && !(client->oldbuttons & BUTTON_ATTACK) && !(client->buttons & BUTTON_ACTIVATE)) {
		Cmd_FollowCycle_f(ent, 1, (client->buttons & BUTTON_SPRINT));
	}
	// FIXME: WBUTTON_ATTACK2
	} else if ((client->wbuttons & WBUTTON_ATTACK2) && !(client->oldwbuttons & WBUTTON_ATTACK2) && !(client->buttons & BUTTON_ACTIVATE)) {
		Cmd_FollowCycle_f(ent, -1, (client->buttons & BUTTON_SPRINT));
	}
#ifdef LEGACY_DEBUG
#ifdef FEATURE_OMNIBOT
	// activate button swaps places with bot
	} else if (client->sess.sessionTeam != TEAM_SPECTATOR && g_allowBotSwap.integer && ((client->buttons & BUTTON_ACTIVATE) && !(client->oldbuttons & BUTTON_ACTIVATE)) && (g_entities[ent->client->sess.spectatorClient].client) && (g_entities[ent->client->sess.spectatorClient].r.svFlags & SVF_BOT)) {
		Cmd_SwapPlacesWithBot_f(ent, ent->client->sess.spectatorClient);
	}
#endif
#endif
	} else if (client->sess.sessionTeam == TEAM_SPECTATOR && client->sess.spectatorState == SPECTATOR_FOLLOW && (((client->buttons & BUTTON_ACTIVATE) && !(client->oldbuttons & BUTTON_ACTIVATE)) || ucmd->upmove > 0) && G_allowFollow(ent, TEAM_AXIS) && G_allowFollow(ent, TEAM_ALLIES)) {
		StopFollowing(ent);
	}
#ifdef FEATURE_MULTIVIEW
}
...
qboolean ClientInactivityTimer(gclient_t *client) {
	...
	if (client->pers.cmd.forwardmove || client->pers.cmd.rightmove || client->pers.cmd.upmove || (client->pers.cmd.wbuttons & (WBUTTON_ATTACK2|WBUTTON_LEANLEFT|WBUTTON_LEANRIGHT)) || (client->pers.cmd.buttons & BUTTON_ATTACK) || (client->ps.eFlags & (EF_MOUNTEDTANK|EF_MG42_ACTIVE|EF_AAGUN_ACTIVE)) || (client->ps.pm_type == PM_DEAD /*&& !(client->ps.eFlags & EF_PLAYDEAD)*/))  // playdead sets PM_DEAD, so check if playing dead ...
	{
		client->inactivityWarning = qfalse;
	...
	if (!client->inactivityWarning) {
		...
		// if a player will not be kicked from the server(because there are still free slots), // do not display messages for inactivity - drop / kick.
		...
		if (level.time > client->inactivityTime) {
			if (inTeam) {
				...
				// slots occupied by bots should be considered "free", // because bots will disconnect if a human player connects..
				if (doDrop) {
...
void SpectatorClientEndFrame(gentity_t *ent) {
	...
	if ((ent->client->sess.spectatorState == SPECTATOR_FOLLOW) || (ent->client->ps.pm_flags & PMF_LIMBO)) {
		...
		if (g_gametype.integer == GT_WOLF_LMS && g_gamestate.integer == GS_PLAYING) {
			...
			if (!level.teamEliminateTime && level.numTeamClients[0] == level.numFinalDead[0] && level.numTeamClients[1] == level.numFinalDead[1] && ent->client->respawnTime <= level.timeCurrent && ent->client->sess.sessionTeam != TEAM_SPECTATOR) {
				do_respawn = qtrue;
			} else {
				do_respawn = qfalse;
			}
	...
	// we are at a free - floating spec state for a player, // set speclock status, as appropriate
