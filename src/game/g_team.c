...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker) {
	...
	{ //      return; // can't find attacker's flag
		...
		if ((VectorLengthSquared(v1) < Square(CTF_TARGET_PROTECT_RADIUS) || VectorLengthSquared(v2) < Square(CTF_TARGET_PROTECT_RADIUS) || CanDamage(flag, targ->client->ps.origin) || CanDamage(flag, attacker->client->ps.origin)) &&
...
qboolean G_readyMatchState(void) {
	...
	if ((g_doWarmup.integer || (g_gametype.integer == GT_WOLF_LMS && g_lms_lockTeams.integer) || level.warmupTime > (level.time + 10 * 1000)) && g_gamestate.integer == GS_WARMUP && G_checkReady()) {
...
void G_verifyMatchState(int nTeam) {
	...
	if ((level.lastRestartTime + 1000) < level.time && (nTeam == TEAM_ALLIES || nTeam == TEAM_AXIS) && (g_gamestate.integer == GS_PLAYING || g_gamestate.integer == GS_WARMUP_COUNTDOWN || g_gamestate.integer == GS_INTERMISSION)) {
