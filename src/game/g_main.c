...
cvarTable_t gameCvarTable[] = {
	...
	// noset vars
	{NULL,          "gamename",     GAMEVERSION, CVAR_SERVERINFO|CVAR_ROM,  0, qfalse}, {NULL,          "gamedate",     __DATE__, CVAR_ROM,                 0, qfalse},
	{&g_restarted,  "g_restarted",  "0",   CVAR_ROM,                 0, qfalse}, {NULL,          "sv_mapname",   "",    CVAR_SERVERINFO|CVAR_ROM,  0, qfalse},
	// latched vars
	...
	{&mod_url,      "mod_url",      "www.etlegacy.com", CVAR_SERVERINFO|CVAR_ROM,  0, qfalse}, // configured by the server admin, points to the web pages for the server
	...
	{&team_artyTime,   "team_artyTime",   "20",  0}, // team class / weapon limiting
	// classes
	...
	{&team_maxCovertops,  "team_maxCovertops",  " - 1",  0,                        0, qfalse, qfalse}, // weapons
	...
	{&team_maxLandmines,  "team_maxLandmines",  "10",  0}, // Skills
	...
	{&g_pronedelay, "g_pronedelay", "0",   CVAR_ARCHIVE|CVAR_SERVERINFO}, // Debug
...
void G_InitGame(int levelTime, int randomSeed, int restart, int legacyServer, int serverVersion) {
	...
	if (g_gametype.integer == GT_WOLF_MAPVOTE) {
		char mapConfig[MAX_STRING_CHARS];
		// trap_Cvar_Set("C", va("%d, %d", //       ((level.mapsSinceLastXPReset >= g_resetXPMapCount.integer) ?
		//               0 : level.mapsSinceLastXPReset) + 1, //       g_resetXPMapCount.integer));
	...
	// always leave room for the max number of clients, // even if they aren't all used, so numbers inside that
...
void G_ShutdownGame(int restart) {
	...
	// gametype latching
	if (
	   ((g_gametype.integer == GT_WOLF || g_gametype.integer == GT_WOLF_CAMPAIGN || g_gametype.integer == GT_WOLF_MAPVOTE) && (g_entities[ENTITYNUM_WORLD].r.worldflags & NO_GT_WOLF)) || (g_gametype.integer == GT_WOLF_STOPWATCH && (g_entities[ENTITYNUM_WORLD].r.worldflags & NO_STOPWATCH)) || (g_gametype.integer == GT_WOLF_LMS && (g_entities[ENTITYNUM_WORLD].r.worldflags & NO_LMS))
) {
...
void MoveClientToIntermission(gentity_t *ent) {
	...
	// Player view is distorted in intermission if you have ridden a vehicle, // mounted a tank
	if (ent->tankLink) {
...
void BeginIntermission(void) {
	...
	if (g_gametype.integer == GT_WOLF_MAPVOTE) {
		...
		// Note: They will never see more than 32 maps in the list(to vote for).
		{
			...
			// I know, perhaps this is a silly way of shuffling the list, // but at least the rest of code stays the same.
		...
		for (teamNum = TEAM_AXIS; teamNum <= TEAM_ALLIES; teamNum++) {
			for (i = 0; i < SK_NUM_SKILLS; i++) {
				...
				if (highestskillpointsclient >= 0) {
					...
					// score. See if there are more clients with this same score. If so, // give them medals too
...
void CheckWolfMP(void) {
	...
	if (g_gametype.integer >= GT_WOLF) {
		...
		case  GS_WARMUP: // check warmup latch
			if (!g_doWarmup.integer || (level.numPlayingClients >= match_minplayers.integer && level.lastRestartTime + 1000 < level.time && G_readyMatchState())) {
		...
		if (level.voteInfo.voteYes > pcnt * total / 100) {
			...
			if ((g_voting.integer & VOTEF_NO_POPULIST_PENALTY)) {
				...
				if (ent->client) {
		          ...
		          level.voteInfo.voteNo >= (100 - pcnt) * total / 100) || level.time - level.voteInfo.voteTime >= VOTE_TIME) {
...
void G_RunThink(gentity_t *ent) {
	...
	if (level.match_pause != PAUSE_NONE && (ent - g_entities) >= g_maxclients.integer && ent->nextthink > level.time && strstr(ent->classname, "DPRINTF_") == NULL) {
...
qboolean G_PositionEntityOnTag(gentity_t *entity, gentity_t *parent, char *tagName) {
	...
	if (entity->client && (entity->s.eFlags & EF_MOUNTEDTANK)) {
		...
		// - fix tank bb
		// - figured out real values, only tag_player is applied, // so there are two left:
