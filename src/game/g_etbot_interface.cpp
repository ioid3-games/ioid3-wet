...
// Note:
// There is a separate goal for soldiers with panzer / bazooka to go defend at
...
static int _GetEntityClass(gentity_t *_ent) {
	...
	case ET_GENERAL:
	{
		...
		else if (!Q_stricmp(_ent->classname, "info_player_deathmatch") || !Q_stricmp(_ent->classname, "team_CTF_redspawn") || !Q_stricmp(_ent->classname, "team_CTF_bluespawn") || !Q_stricmp(_ent->classname, "info_player_spawn")) {
	...
	case ET_PLAYER:
	{
		if (!_ent->client || (_ent->entstate == STATE_INVISIBLE) || (_ent->client->sess.sessionTeam != TEAM_AXIS &&
		...
		/*if (g_gamestate.integer > GS_PLAYING || _ent->client->ps.pm_type > PM_SPECTATOR || _ent->client->ps.pm_flags & PMF_LIMBO)
...
public:
	...
	obResult ChangeTeam(int _client, int _newteam, const MessageHelper *_data) {
		...
		if (_newteam != ET_TEAM_ALLIES && _newteam != ET_TEAM_AXIS) {
			if ((_newteam == RANDOM_TEAM) || (bot->client->sess.sessionTeam != TEAM_AXIS && bot->client->sess.sessionTeam != TEAM_ALLIES)) {
	...
	obResult ChangeClass(int _client, int _newclass, const MessageHelper *_data) {
		...
		if (_newclass <= ET_CLASS_NULL || _newclass >= ET_CLASS_MAX) {
			if ((_newclass == RANDOM_CLASS) || (bot->client->sess.latchPlayerType < 0) || (bot->client->sess.latchPlayerType >= NUM_PLAYER_CLASSES)) {
	...
	void UpdateBotInput(int _client, const ClientInput &_input) {
		...
		if (bot->client->ps.weaponstate == WEAPON_DROPPING_TORELOAD || bot->client->ps.weaponstate == WEAPON_RELOADING || bot->client->ps.weaponstate == WEAPON_RAISING_TORELOAD) {
	...
	obResult TraceLine(obTraceResult &_result, const float _start[3], const float _end[3],  const AABB *_pBBox, int _mask, int _user, obBool _bUsePVS) {
		...
		if (bInPVS) {
			...
			if (_mask & TR_MASK_FLOODFILL) {
				trap_TraceNoEnts(&tr, _start,   _pBBox ? _pBBox->m_Mins : NULL,   _pBBox ? _pBBox->m_Maxs : NULL,   _end, _user, iMask);
			} else {
	...
	GameEntity FindEntityInSphere(const float _pos[3], float _radius, GameEntity _pStart, int classId) {
		...
		if (iHash) {
			...
			while ((pStartEnt = G_FindByClassnameFast(pStartEnt, pClassName, iHash)) != NULL) {
				if (iPlayerClass && pStartEnt->client && pStartEnt->client->sess.sessionTeam != iPlayerClass) {
					continue;
				}
				...
				if (classId == ET_CLASSEX_CORPSE && (!pStartEnt->physicsObject || (pStartEnt->activator && pStartEnt->activator->client->ps.powerups[PW_OPS_DISGUISED]))) {
					continue;
				}
	...
	obResult GetEntityCategory(const GameEntity _ent, BitFlag32 &_category) {
		...
		case ET_GENERAL:
		{
			if (!Q_stricmp(pEnt->classname, "func_invisible_user")) {
				...
				if ((pEnt->health > 0) && (pEnt->entstate != STATE_INVISIBLE) && (pEnt->entstate != STATE_UNDERCONSTRUCTION)) {
		...
		case ET_PLAYER:
		{
			if (!pEnt->client || (pEnt->entstate == STATE_INVISIBLE) || (pEnt->client->ps.pm_type == PM_SPECTATOR) || (pEnt->client->sess.sessionTeam != TEAM_AXIS && pEnt->client->sess.sessionTeam != TEAM_ALLIES)) {
				res = InvalidEntity;
				break;
			}
			// Special case for dead players that haven't respawned.
		...
		/*case ET_AAGUN:
		{
		if ((pCurrent->health > 0) && (pCurrent->entstate != STATE_INVISIBLE) && (pCurrent->entstate != STATE_UNDERCONSTRUCTION)) {
		}
		...
		case ET_EXPLOSIVE:
		{
			if (!(pEnt->spawnflags & EXPLOSIVE_TANK) && (pEnt->constructibleStats.weaponclass != 1) && (pEnt->constructibleStats.weaponclass != 2)) // && // (pEnt->health > 0) && (pEnt->takedamage == qtrue)) {
				_category.SetFlag(ENT_CAT_OBSTACLE);
				_category.SetFlag(ENT_CAT_SHOOTABLE);
			} else {
				res = InvalidEntity;
	...
	obResult GetEntityFlags(const GameEntity _ent, BitFlag64 &_flags) {
		...
		if (pEnt && pEnt->inuse) {
			...
			if ((pEnt->s.eType == ET_MG42_BARREL) || (pEnt->s.eType == ET_GENERAL && !Q_stricmp(pEnt->classname, "misc_mg42"))) {
			...
			case ET_MG42_BARREL:
			{
				if (!Q_stricmp(pEnt->classname, "misc_mg42")) {
					if ((pEnt->health < 0) || (pEnt->entstate == STATE_INVISIBLE)) {
						// _flags.SetFlag(ENT_FLAG_VISTEST);
						_flags.SetFlag(ENT_FLAG_DEAD);
					}
				}
		...
		for (int i = MAX_CLIENTS; i < level.num_entities; ++i) {
			...
			case ET_OID_TRIGGER:
			{
				...
				if (e->target_ent->targetname && (pTmp = strstr(e->target_ent->targetname, "_construct")) != NULL) {
					...
					if (pMover && pMover->s.eType == ET_MOVER && !Q_stricmp(pMover->classname, "script_mover")) {
						continue;
					}
				}
				// / // / // / // / // / // / // / // / // / // / // / // / // / // / // / // / // / // / // / // / // / // / // / ////  / 
				...
				if (e->target_ent->s.eType == ET_CONSTRUCTIBLE) {
					...
					if (bud.udata.m_Int != 0) {
						...
						if (eAxis && eAlly && !Q_stricmp(pAxisGoalName, pAllyGoalName)) {
							Bot_Util_AddGoal("plant", e->target_ent, (1 << ET_TEAM_ALLIES)|(1 << ET_TEAM_AXIS),   pAxisGoalName,   "ExplosiveType",   &bud);
							CheckForMG42(e->target_ent, pAxisGoalName);
						} else {
							if (eAxis) {
								Bot_Util_AddGoal("plant", e->target_ent, (1 << ET_TEAM_ALLIES),   pAxisGoalName,   "ExplosiveType",   &bud);
								CheckForMG42(e->target_ent, pAxisGoalName);
							}

							if (eAlly) {
								Bot_Util_AddGoal("plant", e->target_ent, (1 << ET_TEAM_AXIS),   pAllyGoalName,   "ExplosiveType",   &bud);
					...
					{
						if (eAxis) {
							Bot_Util_AddGoal("explode",   e->target_ent, (1 << ET_TEAM_ALLIES),   pAxisGoalName,   "ExplosiveType",   &bud);
						}

						if (eAlly) {
							Bot_Util_AddGoal("explode",   e->target_ent, (1 << ET_TEAM_AXIS),   pAllyGoalName,   "ExplosiveType",   &bud);
						}
					}
				}
				// check for objective targets
					...
					case 1:
					{
						...
						Bot_Util_AddGoal("satcheltarget", e->target_ent, (1 << ET_TEAM_AXIS), pAxisGoalName);

						if (e->spawnflags & AXIS_OBJECTIVE)
						Bot_Util_AddGoal("satcheltarget", e->target_ent, (1 << ET_TEAM_ALLIES), pAllyGoalName); */
					}
					...
					case 2:
					{
						...
						// note ALLIED / AXIS stuff is reversed here(ET is strange sometimes)
						if (e->spawnflags & ALLIED_OBJECTIVE) {
							Bot_Util_AddGoal("plant",   e->target_ent, (1 << ET_TEAM_AXIS),   pAxisGoalName,   "ExplosiveType",   &bud);
						}

						if (e->spawnflags & AXIS_OBJECTIVE) {
							Bot_Util_AddGoal("plant",   e->target_ent, (1 << ET_TEAM_ALLIES),   pAllyGoalName,   "ExplosiveType",   &bud);
						}
			...
			case ET_MG42_BARREL:
			{
				...
				// 			Bot_Util_AddGoal(e,
	ET_GOAL_MG42MOUNT, team, pGoalName, NULL);
				// 		}
			...
			case ET_MOVER:
			{
				...
				if (pGoalName && !Q_stricmp(e->classname, "script_mover") && e->think != G_FreeEntity) {
		...
		for (int i = 0; i < numofmg42s; ++i) {
			...
			Bot_Util_AddGoal("mountmg42", mg42s[i].ent, (1 << ET_TEAM_ALLIES)|(1 << ET_TEAM_AXIS), strName);
			// cs: this was done in et_goalmanager before
			Bot_Util_AddGoal("repairmg42", mg42s[i].ent, (1 << ET_TEAM_ALLIES)|(1 << ET_TEAM_AXIS), strName);
		}
	}
	...
	obResult InterfaceSendMessage(const MessageHelper &_data, const GameEntity _ent) {
		...
		case GEN_MSG_ISALIVE:
		{
			...
			if (pMsg) {
				if (pEnt && pEnt->inuse && pEnt->client && pEnt->health > 0 && pEnt->client->ps.pm_type == PM_NORMAL) {
					pMsg->m_IsAlive = True;
				}
			}
		...
		case GEN_MSG_ISRELOADING:
		{
			...
			if (pMsg) {
				if ((pEnt && pEnt->inuse && pEnt->client && pEnt->client->ps.weaponstate >= WEAPON_RAISING) && (pEnt->client->ps.weaponstate <= WEAPON_RELOADING)) {
		...
		case GEN_MSG_GETWEAPONLIMITS:
		{
			...
			if (pMsg) {
				if (pEnt && pEnt->inuse && pEnt->client) {
					if (pMsg->m_WeaponId == ET_WP_MOUNTABLE_MG42 && BG_PlayerMounted(pEnt->client->ps.eFlags)) {
						pMsg->m_Limited = True;
		...
		case GEN_MSG_WPCHARGED:
		{
			...
			if (pMsg && pEnt && pEnt->inuse && pEnt->client) {
...
#endif
				{
					...
					pMsg->m_IsCharged =
					   (weaponCharged(&pEnt->client->ps, pEnt->client->sess.sessionTeam,  _weaponBotToGame(pMsg->m_Weapon), pEnt->client->sess.skill) == qtrue) ? True : False;
				}
#else
				pMsg->m_IsCharged =
				   (weaponCharged(&pEnt->client->ps, pEnt->client->sess.sessionTeam,  _weaponBotToGame(pMsg->m_Weapon), pEnt->client->sess.skill) == qtrue) ? True : False;
		...
		case GEN_MSG_MOVERAT:
		{
			...
			if (pMsg) {
			    Vector org(
			    pMsg->m_Position[0], pMsg->m_Position[1], pMsg->m_Position[2]);
			    Vector under(
			    pMsg->m_Under[0], pMsg->m_Under[1], pMsg->m_Under[2]);
		...
		case ET_MSG_GOTOLIMBO:
		{
			...
			if (pMsg) {
				...
				if (pEnt && pEnt->inuse && pEnt->client && pEnt->health > limbo_health && pEnt->client->ps.pm_type == PM_DEAD && !(pEnt->client->ps.pm_flags & PMF_LIMBO)) {
					limbo(pEnt, qtrue);
					pMsg->m_GoLimbo = True;
				} else {
		...
		case ET_MSG_ISMEDICNEAR:
		{
			...
			if (pMsg) {
				if (pEnt && pEnt->inuse && pEnt->client && pEnt->client->ps.pm_type == PM_DEAD && !(pEnt->client->ps.pm_flags & PMF_LIMBO)) {
		...
		case ET_MSG_ISWAITINGFORMEDIC:
		{
			...
			if (pMsg) {
				...
				if (pEnt && pEnt->inuse && pEnt->client && (pEnt->health <= 60 || pEnt->client->ps.pm_type == PM_DEAD) && !(pEnt->client->ps.pm_flags & PMF_LIMBO)) {
		...
		case ET_MSG_GCONSTRUCTABLE:
		{
			...
			if (pMsg) {
				...
				if (pEnt && pEnt->inuse && pConstructable && pConstructable->inuse) {
					...
					} else if ((pConstructable = G_ConstructionForTeam(pConstructable, pEnt->client->sess.sessionTeam)) != NULL && G_ConstructionIsFullyBuilt(pConstructable)) {
				...
				if (pEnt && pEnt->inuse && pDestroyable && pDestroyable->inuse) {
					...
					if (pDestroyable->s.eType == ET_CONSTRUCTIBLE) {
						...
						if (pDestroyable->spawnflags & CONSTRUCTIBLE_INVULNERABLE) {
							...
							while (pCurrent) {
								if (G_ConstructionIsPartlyBuilt(pCurrent) && pCurrent->s.teamNum != pEnt->client->sess.sessionTeam) {
									pMsg->m_State = CONST_DESTROYABLE;
									break;
								}
					...
					} else if (pDestroyable->s.eType == ET_EXPLOSIVE && pDestroyable->parent && pDestroyable->parent->s.eType == ET_OID_TRIGGER && (((pDestroyable->parent->spawnflags & ALLIED_OBJECTIVE) && pEnt->client->sess.sessionTeam == TEAM_AXIS) || ((pDestroyable->parent->spawnflags & AXIS_OBJECTIVE) && pEnt->client->sess.sessionTeam == TEAM_ALLIES))) {
			...
			if (pMsg) {
				...
				if (pEnt && pEnt->client && pFlagEnt) {
					...
					if (pFlagEnt->s.modelindex < 1 || pFlagEnt->s.modelindex >= bg_numItems) {
						...
						pMsg->m_CanBeGrabbed =
						    BG_CanItemBeGrabbed(&pFlagEnt->s, &pEnt->client->ps, pEnt->client->sess.skill, pEnt->client->sess.sessionTeam) ? True : False;
						...
						if (pMsg->m_CanBeGrabbed == True && !(pFlagEnt->flags & FL_DROPPED_ITEM)) {
							if ((pEnt->client->sess.sessionTeam == TEAM_AXIS) && !Q_stricmp(pFlagEnt->classname, "team_CTF_redflag")) {
								GameEntity owner = GetEntityOwner(pMsg->m_Entity);

								if (!owner.IsValid()) {
									pMsg->m_CanBeGrabbed = False;
								}
							} else if ((pEnt->client->sess.sessionTeam == TEAM_ALLIES) && !Q_stricmp(pFlagEnt->classname, "team_CTF_blueflag")) {
								GameEntity owner = GetEntityOwner(pMsg->m_Entity);
		...
		case ET_MSG_GETCVAR:
		{
			...
			if (pMsg) {
				pMsg->m_Value = trap_Cvar_VariableIntegerValue(pMsg->m_Cvar);
			}
	...
	int GetAutoNavFeatures(AutoNavFeature *_feature, int _max) {
		...
		for (int i = MAX_CLIENTS; i < level.num_entities; ++i) {
			...
			if (e->classname) {
				...
				} else if (!Q_stricmp(e->classname, "info_player_deathmatch") || !Q_stricmp(e->classname, "info_player_spawn")) {
...
int Bot_Interface_Init() {
	...
	eomnibot_error err = Omnibot_LoadLibrary(ET_VERSION_LATEST,                  "omnibot_et", Omnibot_FixPath(g_OmniBotPath.string));
...
//#endif
		...
		for (int i = 0; i < g_maxclients.integer; ++i) {
			...
			if ((g_entities[i].client->sess.sessionTeam == TEAM_SPECTATOR) && (g_entities[i].client->sess.spectatorState == SPECTATOR_FOLLOW)) {
...
void Bot_Event_KilledSomeone(int _client, gentity_t *_victim, const char *_meansofdeath) {
	...
	if (IsOmnibotLoaded()) {
		if (IsBot(&g_entities[_client])) {
			...
			Q_strncpyz(d.m_MeansOfDeath, _meansofdeath ? _meansofdeath : " < unknown > ", sizeof(d.m_MeansOfDeath) / sizeof(d.m_MeansOfDeath[0]));
...
void Bot_Event_VoiceMacro(int _client, gentity_t *_source, int _type, const char *_message) {
	...
	if (IsOmnibotLoaded()) {
		if (IsBot(&g_entities[_client])) {
			...
			Event_VoiceMacro d;
			d.m_WhoSaidIt = HandleFromEntity(_source);
			Q_strncpyz(d.m_MacroString, _message ? _message : " < unknown > ", sizeof(d.m_MacroString) / sizeof(d.m_MacroString[0]));
