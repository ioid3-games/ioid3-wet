...
void G_FreeEntity(gentity_t *ed) {
	...
	// before all game entities did relax - now  ET_TEMPHEAD,
	ET_TEMPLEGS and ET_EVENTS no longer relax
			...
			while (*ptr) {
				...
				*mapnamePtr = '\0';
				...
				if (g_gametype.integer == GT_WOLF_CAMPAIGN) {
					if (!mapFound && !Q_stricmp(g_campaigns[level.campaignCount].shortname, g_currentCampaign.string) && !Q_stricmp(mapname, level.rawmapname)) {
				...
				if (g_campaigns[level.campaignCount].mapCount < MAX_MAPS_PER_CAMPAIGN) {
					...
					// yell if there are too many maps in this campaign, // and then skip it
