...
void G_loadMatchGame(void) {
	...
	strcpy(strReinfSeeds, va("%d %d", (dwBlueOffset << REINF_BLUEDELT) + (rand() %(1 << REINF_BLUEDELT)), (dwRedOffset << REINF_REDDELT) + (rand() %(1 << REINF_REDDELT))));
...
static char *pszDPInfo[] = {
	"DPRINTF_PAUSEINFO", "DPRINTF_UNPAUSING", "DPRINTF_CONNECTINFO", "DPRINTF_MVSPAWN", "DPRINTF_UNK1", "DPRINTF_UNK2", "DPRINTF_UNK3", "DPRINTF_UNK4", "DPRINTF_UNK5"
};
...
char *G_createStats(gentity_t *refEnt) {
	...
	for (i = WS_KNIFE; i < WS_MAX; i++) {
		if (refEnt->client->sess.aWeaponStats[i].atts || refEnt->client->sess.aWeaponStats[i].hits || refEnt->client->sess.aWeaponStats[i].deaths || refEnt->client->sess.aWeaponStats[i].kills) {
			dwWeaponMask |= (1 << i);
			Q_strcat(strWeapInfo, sizeof(strWeapInfo), va(" %d %d %d %d %d",                       refEnt->client->sess.aWeaponStats[i].hits, refEnt->client->sess.aWeaponStats[i].atts,                       refEnt->client->sess.aWeaponStats[i].kills, refEnt->client->sess.aWeaponStats[i].deaths,                       refEnt->client->sess.aWeaponStats[i].headshots));
		}
	}
	// Additional info
	...
	Q_strcat(strSkillRatingInfo, sizeof(strSkillRatingInfo), va(" %.2f %.2f",                                     MAX(refEnt->client->sess.mu - 3 * refEnt->client->sess.sigma, 0.f),                                     refEnt->client->sess.mu - 3 * refEnt->client->sess.sigma - (refEnt->client->sess.oldmu - 3 * refEnt->client->sess.oldsigma)));
#endif
...
void G_printMatchInfo(gentity_t *ent) {
	...
	for (i = TEAM_AXIS; i <= TEAM_SPECTATOR; i++) {
		...
		for (j = 0; j < level.numConnectedClients; j++) {
			...
			if (ent->client == cl || (ent->client->sess.sessionTeam == TEAM_SPECTATOR && ent->client->sess.spectatorState == SPECTATOR_FOLLOW && ent->client->sess.spectatorClient == level.sortedClients[j])) {
				ref = "^3";
			}
...
#endif
			      aTeams[i], ref, n2, cl->sess.time_axis / 60000, cl->sess.time_allies / 60000, ref, time_eff, cl->sess.kills, cl->sess.deaths, cl->sess.gibs, cl->sess.self_kills, cl->sess.team_kills, cl->sess.team_gibs, ref, eff, cl->sess.damage_given, cl->sess.damage_received, cl->sess.team_damage_given, cl->sess.team_damage_received, cl->ps.persistant[PERS_SCORE]
#ifdef FEATURE_RATING
			     , (cl->sess.mu - 3 * cl->sess.sigma < 0.f) ? 0.f : cl->sess.mu - 3 * cl->sess.sigma, (cl->sess.mu - 3 * cl->sess.sigma) - (cl->sess.oldmu - 3 * cl->sess.oldsigma)
#endif
		));
		}
...
#endif
		      aTeams[i], "Totals", tot_timex / 60000, tot_timel / 60000, time_eff, tot_kills, tot_deaths, tot_gibs, tot_sk, tot_tk, tot_tg, eff, tot_dg, tot_dr, tot_tdg, tot_tdr, tot_xp
#ifdef FEATURE_RATING
		     , (TeamCount(-1, i) == 0) ? 0 : ((tot_rating < 0.f) ? 0.f : tot_rating / TeamCount(-1, i)), (TeamCount(-1, i) == 0) ? 0 : tot_delta / TeamCount(-1, i)
#endif
	));
	}
