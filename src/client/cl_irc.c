...
static ID_INLINE void IRC_InitHandlers() {
	IRC_Handlers = HT_Create(100, HT_FLAG_INTABLE|HT_FLAG_CASE,  sizeof(struct irc_handler_t),  HT_OffsetOfField(struct irc_handler_t, cmd_string),  32);
	IRC_CTCPHandlers = HT_Create(100, HT_FLAG_INTABLE|HT_FLAG_CASE,      sizeof(struct irc_handler_t),      HT_OffsetOfField(struct irc_handler_t, cmd_string),      32);
}
...
static qboolean IRC_Parser(char next) {
	...
	*and control characters which all cause an error recovery.
	...
	case IRC_PARSER_PFX_NOS_START:
		...
		break;
	/*
	*Prefix, server or nick name. Control characters cause an error, * ' ','!' and '@' cause state changes.
	 */
	...
	*End of numeric command, could be a ' ' or a '\r'.
	...
	case IRC_PARSER_NUM_COMMAND_4:
		...
		break;
	/*
	*Start of parameter. ':' means it's a trailing parameter, * spaces and control characters shouldn't be here, and
	*anything else is a "middle" parameter.
	...
	case IRC_PARSER_PARAM_START:
		...
		break;
	/*
	*"Middle" parameter; ' ' means there's another parameter coming, * '\r' means the end of the message, control characters are not
	*accepted, anything else is part of the parameter.
	...
	case IRC_PARSER_MID_PARAM:
		...
		break;
	/*
	*Trailing parameter; '\r' means the end of the command, * and anything else is just added to the string.
	 */
...
static qboolean IRC_InitialConnect() {
	...
	do {
		// If we're re - attempting a connection, wait a little bit, // or we might just piss the server off.
