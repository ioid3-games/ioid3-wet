...
static qboolean S_AL_HearingThroughEntity(int entityNum) {
	...
	if (clc.clientNum == entityNum) {
		...
		distanceSq = vec3_distance_squared(
		    entityPositions[entityNum], lastListenerOrigin);
...
static srcHandle_t S_AL_SrcAlloc(alSrcPriority_t priority, int entnum, int channel) {
	...
	for (i = 0; i < srcCount; i++) {
		...
		if (curSource->isPlaying) {
			if (weakest_isplaying && curSource->priority < priority && (curSource->priority < weakest_pri ||
...
static void S_AL_StartSoundEx(vec3_t origin, int entnum, int entchannel, sfxHandle_t sfx, int flags, int volume) {
	...
	if ((srcActiveCnt > 5 * srcCount / 3) && (vec3_distance_squared(sorigin, lastListenerOrigin) > =
...
void S_AL_FadeStreamingSound(float targetVol, int time, int ss) {
	...
	if (ssData[ss].fadeStart) {
		ssData[ss].fadeStartVol = (ssData[ss].fadeEnd <= currentTime) ? ssData[ss].fadeTargetVol : ((float) (currentTime - ssData[ss].fadeStart) / (float) (ssData[ss].fadeEnd - ssData[ss].fadeStart));
	}
...
qboolean S_AL_Init(soundInterface_t *si) {
	...
	if (!s_alCapture->integer) {
		...
		// !!! FIXME: so we'll just check the function pointer for now.		qboolean test = qalcCaptureOpenDevice == NULL;
#else
