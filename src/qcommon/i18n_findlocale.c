...
FL_Success FL_FindLocale(FL_Locale **locale) {
	...
	// assume unixoid
	{
		...
		if (accumulate_env("LC_ALL", rtn) || accumulate_env("LC_MESSAGES", rtn) || accumulate_env("LANG", rtn) || accumulate_env("LANGUAGE", rtn)) {
			success = FL_CONFIDENT;
		}
