...
void RB_AddQuadStampFadingCornersExt(vec3_t origin, vec3_t left, vec3_t up, byte *color, float s1, float t1, float s2, float t2) {
	...
	*( unsigned int * ) &tess.vertexColors[ndx + 4].v =
	...
	lColor[3] = 0;
	*( unsigned int * ) &tess.vertexColors[ndx].v = *( unsigned int * ) &tess.vertexColors[ndx + 1].v = *( unsigned int * ) &tess.vertexColors[ndx + 2].v = *( unsigned int * ) &tess.vertexColors[ndx + 3].v = *( unsigned int * )lColor;
