...
void Sys_CreateConsole(void) {
	...
	s_wcd.hWnd = CreateWindowEx(0,     DEDCLASS,     WINDOWNAME,     DEDSTYLE,    (swidth - 600) / 2, (sheight - 450) / 2, rect.right - rect.left + 1, rect.bottom - rect.top + 1,     NULL,     NULL,     g_wv.hInstance,     NULL);
	...
	s_wcd.hwndInputLine = CreateWindowEx(WS_EX_CLIENTEDGE,              "edit", NULL, WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT|ES_AUTOHSCROLL,              0, 0, 0, 0,              s_wcd.hWnd,             (HMENU) INPUT_ID, // child window ID
	                                     g_wv.hInstance, NULL);
	// create the buttons
	s_wcd.hwndButtonCopy = CreateWindow("button", NULL, BS_PUSHBUTTON|WS_VISIBLE|WS_CHILD,             0, 0, 0, 0,             s_wcd.hWnd,            (HMENU) COPY_ID, // child window ID
	                                    g_wv.hInstance, NULL);
	SendMessage(s_wcd.hwndButtonCopy, WM_SETTEXT, 0, (LPARAM) "Copy");

	s_wcd.hwndButtonClear = CreateWindow("button", NULL, BS_PUSHBUTTON|WS_VISIBLE|WS_CHILD,              0, 0, 0, 0,              s_wcd.hWnd,             (HMENU) CLEAR_ID, // child window ID
	                                     g_wv.hInstance, NULL);
	SendMessage(s_wcd.hwndButtonClear, WM_SETTEXT, 0, (LPARAM) "Clear");

	s_wcd.hwndButtonQuit = CreateWindow("button", NULL, BS_PUSHBUTTON|WS_VISIBLE|WS_CHILD,             0, 0, 0, 0,             s_wcd.hWnd,            (HMENU) QUIT_ID, // child window ID
	                                    g_wv.hInstance, NULL);
	SendMessage(s_wcd.hwndButtonQuit, WM_SETTEXT, 0, (LPARAM) "Quit");
	// create the scrollbuffer
	s_wcd.hwndBuffer = CreateWindowEx(WS_EX_CLIENTEDGE,           "edit", NULL, WS_CHILD|WS_VISIBLE|WS_VSCROLL /*|WS_BORDER*/ |
	                                  ES_LEFT|ES_MULTILINE|ES_AUTOVSCROLL|ES_READONLY,           0, 0, 0, 0,           s_wcd.hWnd,          (HMENU) EDIT_ID, // child window ID
	                                  g_wv.hInstance, NULL);
#if defined(_WIN64)
...
void Sys_SetErrorText(const char *buf) {
	...
	if (!s_wcd.hwndErrorBox) {
		...
		s_wcd.hwndErrorBox = CreateWindow("static", NULL, WS_CHILD|WS_VISIBLE|SS_SUNKEN,           0, 0, 0, 0,           s_wcd.hWnd,          (HMENU)ERRORBOX_ID, // child window ID
		                                  g_wv.hInstance, NULL);
