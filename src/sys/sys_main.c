...
void Sys_AnsiColorPrint(const char *msg) {
	...
	static int etAnsiHash[][6] =
	{
		...
		{1, 30, 0, '0','p','P'}, {1, 31, 0, '1','q','Q'}, {1, 32, 0, '2','r','R'}, {1, 33, 0, '3','s','S'}, {1, 34, 0, '4','t','T'}, {1, 36, 0, '5','u','U'}, {1, 35, 0, '6','v','V'}, {1, 37, 0, '7','w','W'},
		...
		{38, 5, 197, 'i','I',')'}, {38, 5, 124, 'j','J','*'}, {38, 5, 130, 'k','K','+'}, {38, 5, 179, 'l','L',', '}, {38, 5, 143, 'm','M','-'}, {38, 5, 229, 'n','N','.'}, {38, 5, 228, 'o','O','/'}
	};
...
static void *Sys_TryLibraryLoad(const char *base, const char *gamedir, const char *fname) {
	...
	// This is why .bundles were used instead. When W:ET originally shipped, it used used .bundle packages for the VM libraries, // but to make them single files, it zipped that .bundle package and had no extension at all(ie just "cgame_mac"). But now
...
void *Sys_LoadGameDll(const char *name, qboolean extract, 
	...
	// lib must either already be in the homepath, or in the basepath, // without being in a pak. For a pure server, it always grabs the lib
...
#ifdef FEATURE_CURSES
	for (i = 1; i < argc; i++) {
		if (!strcmp(argv[i], " + nocurses") || !strcmp(argv[i], "--nocurses")) {
			nocurses = qtrue;
			break;
		}
	}
