...
POParser::POParser(const std::string& filename_, std::istream& in_, Dictionary& dict_, bool use_fuzzy_) :
	filename(filename_), in(in_), dict(dict_), use_fuzzy(use_fuzzy_), running(false), eof(false), big5(false), line_number(0), current_line(), conv() {
}
...
void POParser::get_string_line(std::ostringstream& out, size_t skip) {
...
void POParser::parse() {
	...
	if (current_line.size() >= 3 && current_line[0] == static_cast < char > (0xef) && current_line[1] == static_cast < char > (0xbb) && current_line[2] == static_cast < char > (0xbf)) {
...
next:
					...
					} else if (prefix("msgstr[") && current_line.size() > 8 && isdigit(current_line[7]) && current_line[8] == ']') {
