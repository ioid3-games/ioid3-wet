...
void IConv::set_charsets(const std::string& from_charset_, const std::string& to_charset_) {
...
IConv::convert(const std::string& text) {
	...
	if (!cd) {
		...
		if (ret == static_cast < size_t > (-1)) {
			if (errno == EILSEQ || errno == EINVAL) {// invalid multibyte sequence
			...
			} else if (errno == E2BIG) {// output buffer to small
