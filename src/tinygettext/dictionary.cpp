...
Dictionary::Dictionary(const std::string& charset_) :
	entries(), ctxt_entries(), charset(charset_), plural_forms(), m_has_fallback(false), m_fallback() {
}
...
Dictionary::translate_plural(const Entries& dict, const std::string& msgid, const std::string& msgid_plural, int count) const
{
	...
	if (it != dict.end()) {
		...
		if (!msgstrs[n].empty()) {
			return msgstrs[n];
		} else if (count == 1) // default to english rules
		{
			return msgid;
		} else {
			return msgid_plural;
...
void Dictionary::add_translation(const std::string& msgid, const std::string& msgid_plural, 
...
void Dictionary::add_translation(const std::string& msgctxt, const std::string& msgid, const std::string& msgstr) {
	std::vector < std::string > & vec = ctxt_entries[msgctxt][msgid];
