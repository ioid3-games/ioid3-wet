...
static void SVC_Status(netadr_t from, qboolean force) {
	...
	Info_SetValueForKey(infostring, "version",
	ET_VERSION);
...
qboolean SV_CheckDRDoS(netadr_t from) {
	...
	for (i = 0; i < MAX_INFO_RECEIPTS; i++, receipt++) {
		if (receipt->time + 2000 > timeNow) {
			if (receipt->time) {
				// When the server starts, all receipt times are at zero. Furthermore, // svs.time is close to zero. We check that the receipt time is already
