...
qboolean SV_DemoClientCommandCapture(client_t *client, const char *msg) {
	...
	} else if (sv.demoState == DS_PLAYBACK && ((client - svs.clients) >= sv_democlients->integer) && ((client - svs.clients) < sv_maxclients->integer) && // preventing only real clients commands(not democlients commands replayed)
...
static void SV_DemoReadClientConfigString(msg_t *msg) {
	...
	if (Q_stricmp(sv.configstrings[CS_PLAYERS + num], configstring) && configstring && strlen(configstring)) {
		...
		if (!strlen(Info_ValueForKey(svs.clients[num].userinfo, "team"))) {
			if (configstring && strlen(configstring) && (svdoldteam == -1 || (svdoldteam != svdnewteam && svdnewteam != -1)) // if there was no team for this player before or if the new team is different
		) {
...
static void SV_DemoReadClientUserinfo(msg_t *msg) {
	...
	if (strlen(userinfo) && strlen(svdnewteam) && (!strlen(svdoldteam) || (Q_stricmp(svdoldteam, svdnewteam) && strlen(svdnewteam))) // if there was no team for this player before OR if the new team is different
) {
...
read_next_demo_event: // used to read next demo event
		...
		while (1) {
			...
			case demo_clientCommand:    // client command management(generally automatic, such as tinfo for HUD team overlay status, team selection, etc.) - except userinfo command that is managed by another event
				...
				break;
			case demo_serverCommand:    // server command management - except print / cp(already handled by gameCommand), SV_DemoReadServerCommand(&msg);
				break;
