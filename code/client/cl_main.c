...
Update cl_guid using #ETKEY_FILE.
=======================================================================================================================================
*/
...
Test for the existence of a valid #ETKEY_FILE and generate a new one if it doesn't exist.
=======================================================================================================================================
*/
...
Send motd request to the #MOTD_SERVER_NAME.
=======================================================================================================================================
*/
...
void CL_Vid_Restart_f(void) {
	...
	// don't show percent bar, since the memory usage will just sit at the same level anyway - so keep the value - feels like a bug for users
...
void CL_DownloadsComplete(void) {
	...
	// starting to load a map so we get out of full screen ui mode
	Cvar_Set("r_uiFullScreen", "0");
	// flush client memory and start loading stuff, this will also (re)load the UI
...
Sometimes the server can drop the client and the netchan based disconnect can be lost. If the client continues to send packets to the
server, the server will send out of band disconnect packets to the client so it doesn't have to wait for the full timeout period.
=======================================================================================================================================
*/
...
Client received 'motd' connectionless packet.
=======================================================================================================================================
*/
...
An OOB message from server, with potential markups. Print OOB are the only messages we handle markups in.
	[err_dialog]: Used to indicate that the connection should be aborted. No further information, just do an error diagnostic screen
	afterwards.
	[err_prot]:	This is a protocol error. The client uses a custom protocol error message(client sided) in the diagnostic window.
...
Responses to broadcasts, etc. Compare first n chars so it doesn't bail if token is parsed incorrectly.
=======================================================================================================================================
*/
void CL_ConnectionlessPacket(netadr_t from, msg_t *msg) {
	...
	// a disconnect message from the server, which will happen if the server dropped the connection but it is still getting packets from us
...
void CL_CheckTimeout(void) {
	...
	// check timeout
	if ((!cl_paused->integer || !sv_paused->integer) && cls.state >= CA_CONNECTED && cls.state != CA_CINEMATIC && cls.realtime - clc.lastPacketTime > cl_timeout->value * 1000 && !(clc.demoplaying && cl_freezeDemo->integer)) {
...
void CL_Frame(int msec) {
	...
	} else if ((!cl_avidemo->integer && CL_VideoRecording()) || (cl_avidemo->integer && (cls.state != CA_ACTIVE || !cl_forceavidemo->integer))) {
		CL_StopVideo_f();
	}
...
void CL_Init(void) {
	...
	// init autoswitch so the ui will have it correctly evenif the cgame hasn't been started
...
Sends a request for server list to the chosen master server. 0 fetch all master servers and 1 - 5 request a single master server.
=======================================================================================================================================
*/
...
Called by engine to handle translation.
...
/*
=======================================================================================================================================
CL_TranslateStringMod

Called by mod libs to handle translation.
