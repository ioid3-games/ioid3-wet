...
void Console_Key(int key) {
	...
	if ((key >= K_SPACE && key <= K_BACKSPACE) || (key == K_LEFTARROW) || (key == K_RIGHTARROW) || (key >= K_KP_LEFTARROW && key <= K_KP_RIGHTARROW) || (key >= K_KP_SLASH && key <= K_KP_PLUS) || (key >= K_KP_STAR && key <= K_KP_EQUALS)) {
	...
	if ((key == K_MWHEELUP && (keys[K_RSHIFT].down || keys[K_LSHIFT].down)) || (key == K_UPARROW) || (key == K_KP_UPARROW) || ((tolower(key) == 'p') && (keys[K_LCTRL].down || keys[K_RCTRL].down))) {
	...
	// added some mousewheel functionality to the console
	if ((key == K_MWHEELDOWN && (keys[K_RSHIFT].down || keys[K_LSHIFT].down)) || (key == K_DOWNARROW) || (key == K_KP_DOWNARROW) || ((tolower(key) == 'n') && (keys[K_LCTRL].down || keys[K_RCTRL].down))) {
...
void Key_SetBinding(int keynum, const char *binding) {
	char *lcbinding;
	// make a copy of our binding lowercase,so name toggle scripts work again: bind x name BzZIfretn?
...
void CL_KeyEvent(int key, qboolean down, unsigned time) {
	...
	if (key == K_ENTER) {
		if (down) {
			if (keys[K_LALT].down || keys[K_RALT].down) {
				...
				if (Q_stristr(cls.glconfig.renderer_string, "mesa") || Q_stristr(cls.glconfig.renderer_string, "gallium") || Q_stristr(cls.glconfig.vendor_string, "nouveau") || Q_stristr(cls.glconfig.vendor_string, "mesa")) {
	...
	// most keys during demo playback will bring up the menu, but non-ascii keys can still be used for bound actions
	...
	if (!down) {
		...
		if (kb && kb[0] == ' + ') {
			// button commands add keynum and time as parms so that multiple sources can be discriminated and subframe corrected
