...
/*
=======================================================================================================================================
CM_TraceThroughBounds
=======================================================================================================================================
*/
static int CM_TraceThroughBounds(traceWork_t *tw, vec3_t mins, vec3_t maxs) {

	if (mins[0] > tw->bounds[1][0] || maxs[0] < tw->bounds[0][0] || mins[1] > tw->bounds[1][1] || maxs[1] < tw->bounds[0][1] || mins[2] > tw->bounds[1][2] || maxs[2] < tw->bounds[0][2]) {
		return qfalse;
	}
...
static void CM_TraceThroughTree(traceWork_t *tw, int num, float p1f, float p2f, vec3_t p1, vec3_t p2) {
	...
	if (plane->type < 3) {
		...
		if (tw->isPoint) {
			...
			/*
			// an axial brush right behind a slanted bsp plane will poke through when expanded, so adjust by sqrt(3)
			offset = Q_fabs(tw->extents[0]*plane->normal[0]) + Q_fabs(tw->extents[1]*plane->normal[1]) + Q_fabs(tw->extents[2]*plane->normal[2]);
			offset *= 2;
			offset = tw->maxOffset;
			*/
			offset = tw->maxOffset;
...
=======================================================================================================================================
*/
static void CM_Trace(trace_t *results, const vec3_t start, const vec3_t end, const vec3_t mins, const vec3_t maxs, clipHandle_t model, const vec3_t origin, int brushmask, qboolean capsule, sphere_t *sphere) {
	int i;
	traceWork_t tw;
	vec3_t offset;
	cmodel_t *cmod;
...
void CM_TransformedBoxTrace(trace_t *results, const vec3_t start, const vec3_t end, const vec3_t mins, const vec3_t maxs, clipHandle_t model, int brushmask, const vec3_t origin, const vec3_t angles, qboolean capsule) {
	...
	if (rotated) {
		...
		// NOTE: This is still incorrect for bounding boxes because the actual bounding box that is swept through the model is not rotated.
		//		 We cannot rotate the bounding box or the bmodel because that would make all the brush bevels invalid.
