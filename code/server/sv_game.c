...
void SV_GameSendServerCommand(int clientNum, const char *text) {
	...
	if (sv.demoState == DS_RECORDING) {
		...
		// store the new game command, so when replaying a demo message, we can check for duplicates: maybe this message was already
		// submitted (because of the events simulation, an event may trigger a message), and so we want to avoid those duplicates
		// if an event already triggered a message, no need to issue the one stored in the demo
...
Return 1 if message is in queue - 0 not sent.
=======================================================================================================================================
*/
...
intptr_t SV_GameSystemCalls(intptr_t *args) {
	...
	switch (args[0]) {
		...
		case G_SET_CONFIGSTRING:
			// don't allow the game to overwrite demo configstrings (unless it modifies the normal spectator clients configstrings, this exception allows for player connecting
			// during a demo playback to be correctly rendered, else they will get an empty configstring so no icon, no name, nothing...)
