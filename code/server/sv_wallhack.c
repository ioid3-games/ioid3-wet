...
/*
=======================================================================================================================================
predict_clip_velocity

The following functions for predicting player positions have been adopted from 'g_unlagged.c' which is part of the 'unlagged' system
created by Neil "haste" Toronto. WEB site: http://www.ra.is/unlagged.
=======================================================================================================================================
*/
...
static void predict_move(sharedEntity_t *ent, float frametime, trajectory_t *tr, vec3_t result) {
	...
	SV_Trace(&trace, start_o, ent->r.mins, ent->r.maxs, up, ent->s.number, CONTENTS_SOLID, qfalse);
	...
	SV_Trace(&trace, tr->trBase, ent->r.mins, ent->r.maxs, down, ent->s.number,  CONTENTS_SOLID, qfalse);
...
Calculates the view point of a player model at position 'org' using information in the player state 'ps' of its client, and stores the
...
static int player_in_fov(vec3_t viewangle, vec3_t ppos, vec3_t opos) {
	...
	// check if the two players are roughly on the same X/Y plane and skip the test if not. We only want to eliminate info that
	// would reveal the position of opponents behind the player on the same X/Y plane (e.g. on the same floor in a room).
...
Checks if 'player' can see 'other' or not.

First a check is made if 'other' is in the maximum allowed fov of 'player'. If not, then zero is returned w/o any further checks.
Next traces are carried out from the present viewpoint of 'player' to the corners of the bounding box of 'other'. If any of these
traces are successful (i.e. nothing solid is between the start and end positions) then non-zero is returned.

Otherwise the expected positions of the two players are calculated, by extrapolating their movements for PREDICT_TIME seconds and the
above tests are carried out again. The result is reported by returning non-zero (expected to become visible) or zero (not expected to
become visible in the next frame).
=======================================================================================================================================
*/
int SV_CanSee(int player, int other) {
	...
	// FIXME: We use the original viewangle that may have changed during the move. This could introduce some errors.
	if (sv_wh_check_fov->integer > 0) {
...
Changes the position of client 'other' so that it is directly below 'player'. The distance is maintained so that sound scaling will
work correctly.
=======================================================================================================================================
*/
